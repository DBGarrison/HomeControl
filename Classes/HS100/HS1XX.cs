...
namespace HomeControl.HS100
{
    ...
    {
		private static byte[] cmdGetSysInfo = new byte[] { 0, 0, 0, 0, 208, 242, 129, 248, 139, 255, 154, 247, 213, 239, 148, 182, 209, 180, 192, 159, 236, 149, 230, 143, 225, 135, 232, 202, 240, 158, 235, 135, 235, 150, 235 };
		private static byte[] cmdRelayOff = new byte[] { 0, 0, 0, 0, 208, 242, 129, 248, 139, 255, 154, 247, 213, 239, 148, 182, 197, 160, 212, 139, 249, 156, 240, 145, 232, 183, 196, 176, 209, 165, 192, 226, 216, 163, 129, 242, 134, 231, 147, 246, 212, 238, 222, 163, 222, 163 };
		private static byte[] cmdRelayOn = new byte[] { 0, 0, 0, 0, 208, 242, 129, 248, 139, 255, 154, 247, 213, 239, 148, 182, 197, 160, 212, 139, 249, 156, 240, 145, 232, 183, 196, 176, 209, 165, 192, 226, 216, 163, 129, 242, 134, 231, 147, 246, 212, 238, 223, 162, 223, 162 };
		...
		public HS1XX(int deviceId, IPAddress pIP, int pConnectionTimeOut = 1000, int pSendCommandTimeOut = 1000, int pReceiveCommandTimeOut = 1000)
		{
		...
		private void ExecuteCommand(string pCommand)
        {
            ...
            Common.showCommandAndBytes(pCommand, mEncryptedMessage);
        ...
        private string ExecuteAndRead(byte[] mEncryptedMessage)
        {
        ...
        */
        ...
        {
            DeviceInfo dev_info = new DeviceInfo(this.DeviceId);

            //Prepare the command
            //string comando = "{\"system\":{\"get_sysinfo\":null}}";

            //Send the command to the device and get the Json string result
            string sJsonResult = this.ExecuteAndRead(cmdGetSysInfo);
		...
		public static Dictionary<IPAddress, DeviceInfo> GetAllDevices_TCP_Scanner(NetworkInterfaceType pNetworkType,                                                                                   
                                                                                  int pMillisecondsTimeOut = 10)
        {
        ...
        private byte[] getCommandBytes(int cmd)
        {
            if (cmd == 0)
            {
                return new byte[]{0, 0, 0, 0,208, 242,129, 248,139,255,154,247,213,239,148,182,197,160,212,139,
                                    249,156,240,145,232,183,196,176,209,165,192,226,216,163,129,242,134,
                                        231,147,246,212,238,222,163,222,163 };
            }
            else if (cmd == 1)
            {
                return new byte[]{0, 0, 0, 0,208, 242,129, 248,139,255,154,247,213,239,148,182,197,160,212,139,
                                    249,156,240,145,232,183,196,176,209,165,192,226,216,163,129,242,134,
                                        231,147,246,212,238,223,162,223,162 };
            }
            else if (cmd == 2)
            {
